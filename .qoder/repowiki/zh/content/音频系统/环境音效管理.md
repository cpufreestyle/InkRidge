...
- Environment：AmbientAudio、AmbientBreathFilter
...
- [BreathGuide.cs:1-36](file://Assets/Scripts/Meditation/BreathGuide.cs#L1-L36)
- [AudioSetup.cs:1-35](file://Assets/Editor/AudioSetup.cs#L1-L35)
...
- MeditationPoint：触发冥想会话，协调视觉、触觉、音频与滤镜；在开始/结束时调用 AmbientAudio.Duck/Restore。
- AmbientBreathFilter：为环境音添加低通滤波器，随呼吸阶段开合，增强沉浸感。
...
## 架构总览
AmbientAudio 作为场景环境音的“单一事实源”，通过协程实现平滑的音量过渡；MeditationPoint 在冥想开始/结束时分别调用 Duck 与 Restore；AudioMixerController 则从全局层面在冥想时进一步压低环境音量；AmbientBreathFilter 根据呼吸阶段调节低通滤波，形成“声音呼吸”。
...
与 AmbientAudio 的交互
- 开始冥想：_ambientAudio.Duck(_meditationAmbientVolume, _duckFadeSeconds)。
- 结束冥想：_ambientAudio.Restore()。
...
- AmbientBreathFilter 依赖 BreathGuide 与 AudioLowPassFilter。
...
## 结论
AmbientAudio 提供了简洁而强大的场景环境音管理能力，通过协程与线性插值实现平滑的淡入淡出，并与 MeditationPoint、AudioMixerController、AmbientBreathFilter 协同工作，在冥想期间有效压低环境音，确保呼吸指导清晰可闻。配合合理的资源复用与内存管理策略，可在 VR 设备上获得稳定流畅的音频体验。
...
- AmbientBreathFilter
  - _openCutoff/_closedCutoff：低通滤波器的开/关截止频率
  - _responseSpeed：响应速度
