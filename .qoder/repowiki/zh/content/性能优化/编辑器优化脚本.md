...
## 更新摘要
**所做更改**
- 增强了OptimizeProject脚本的核心优化功能
- 新增着色器保护系统和Always Included Shaders机制
- 添加了单通道实例化的安全检测与启用功能
- 实现了智能音频优化系统
- 改进了质量配置的精确控制
- 增强了构建流程的工具链管理
...
**更新** OptimizeProject脚本现已大幅增强，新增了着色器保护系统、高级质量配置、单通道实例化准备和音频优化功能，为Quest 3平台提供了完整的优化解决方案。
...
M["着色器保护系统"] --> N["Always Included Shaders"]
O["音频优化系统"] --> P["AudioImporter配置"]
```
...
## 核心组件
- **OptimizeProject**：提供"Debug > Optimize Project for Quest 3"菜单项，统一调整质量等级、玩家设置、图形常量着色器、音频导入配置，并持久化到资源数据库。现已支持着色器保护、单通道实例化检测和智能音频优化。
...
**更新** 新的架构包含了着色器保护系统和音频优化模块，形成了更完整的优化生态系统。
...
**更新** 现已包含着色器保护系统、高级质量配置、单通道实例化准备和音频优化功能。

#### 着色器保护系统
- **Always Included Shaders机制**：防止构建裁剪导致运行时找不到着色器而回退错误材质
- **支持的着色器**：Gongbi/Toon、Gongbi/InkSkybox、Gongbi/BreathCircle、Gongbi/BreathGlow、Gongbi/InkFog、Hidden/Vignette
- **动态注册**：自动扫描并注册所有必需的着色器到GraphicsSettings

#### 高级质量配置
- **解析或回退到最高质量等级**，关闭阴影系统、限制像素灯数量、低分辨率阴影、禁用级联、开启各向异性过滤、软植被、禁用软粒子与实时反射探针、降低粒子预算、LOD 偏置、关闭 VSync（VR 合成器控制）
- **将当前质量等级设置为 Android 默认质量**，确保跨平台一致性
...
#### 单通道实例化准备
- **安全检测机制**：检查所有Gongbi系列着色器是否包含必要的立体实例化宏
- **缺失宏检测**：UNITY_VERTEX_INPUT_INSTANCE_ID、UNITY_VERTEX_OUTPUT_STEREO
- **拒绝切换保护**：如果着色器未准备好，拒绝启用单通道实例化并列出缺失文件

#### 音频优化
- **智能音频分类**：根据文件名识别环境音（ambience）采用流式加载，其他音效解压加载
- **统一压缩格式**：Vorbis压缩与中等质量（0.7f）
- **内存优化**：环境音流式加载避免长音频驻留内存，短音效解压加载确保即时播放
...
AudioCfg --> ShaderCheck["检查着色器实例化宏"]
ShaderCheck --> SaveRefresh["保存并刷新资源"]
SaveRefresh --> End(["结束"])
```
...
- **内存与体积**
  ...
  - **着色器保护**：Always Included Shaders避免运行时查找开销
- **帧时序**
...
- **着色器保护失败**
  - 现象：某些着色器未被正确注册到Always Included列表
  - 处理：检查着色器名称是否正确，确认着色器文件存在且可被Shader.Find定位
- **音频优化异常**
  - 现象：音频文件未被正确优化或加载类型设置错误
  - 处理：检查Assets/Audio目录结构，确认音频文件存在且可被AssetDatabase访问
- **构建工具链缺失**
...
## 结论
InkRidge 的编辑器工具集以 OptimizeProject 为核心，结合 BuildAPK、SceneSetup 与若干场景装配脚本，形成从"编辑期一键优化"到"构建期稳定打包"的完整工作流。**更新的OptimizeProject脚本现在包含了着色器保护系统、高级质量配置、单通道实例化准备和音频优化功能**，为Quest 3等移动VR平台提供了更加完善和安全的优化解决方案。通过严格的质量与玩家设置、安全的着色器裁剪保护、以及智能的音频导入优化，项目在Quest 3等移动VR平台上获得稳定的性能与体积表现。建议团队在新增内容时遵循既定流程，必要时扩展自定义优化规则，并通过基准测试持续验证收益。

## 附录

### 不同平台优化策略
**更新** 增加了着色器保护和音频优化的平台特定考虑。
...
- **Android（Quest 3）**
  ...
  - **着色器保护**：自动检测并注册所有必需着色器到Always Included列表
- **iOS**
...
### 性能基准测试方法
**更新** 增加了着色器保护和音频优化的性能指标。

- **指标采集**
  - 使用 Unity Profiler 采集 CPU/GPU 耗时、DrawCalls、三角面数、内存占用
  - 在 Quest 3 设备上进行真实负载测试，关注首帧与稳态帧率
  - **着色器保护性能**：监控Always Included Shaders的加载时间和内存占用
  - **音频优化效果**：测量音频加载时间和内存峰值
- **对比实验**
  - 基线：默认质量与设置
  - 优化后：运行 OptimizeProject 后的配置
  - 变量：开启/关闭单通道实例化、MSAA、软粒子等
  - **着色器保护测试**：比较启用/禁用Always Included Shaders的性能差异
  - **音频优化测试**：对比不同音频压缩格式和加载类型的效果
- **回归监控**
  - 将关键指标纳入 CI 门禁，当帧率下降或包体增长超过阈值时告警
  - **着色器完整性检查**：确保所有必需着色器都被正确注册
  - **音频文件大小监控**：跟踪音频优化对包体的影响

### 编辑器集成最佳实践
**更新** 增加了着色器保护和音频优化的集成指导。
...
- **记录变更日志与输出信息**，便于问题追踪
- **着色器保护集成**：确保着色器文件路径正确，提供着色器缺失时的友好提示
- **音频优化集成**：支持增量优化，只处理新增或修改的音频文件
